        ...
        return if (builder.remaining.startsWith("#")) {
            SharedSuggestionProvider.suggestResource(
                mutableListOf<ResourceLocation>().apply {
                    addAll(BuiltInRegistries.BLOCK.tags.map { it.first.location }.toList())
                    addAll(BuiltInRegistries.ITEM.tags.map { it.first.location }.toList())
                    addAll(BuiltInRegistries.ENTITY_TYPE.tags.map { it.first.location }.toList())
                },
